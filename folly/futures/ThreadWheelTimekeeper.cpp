...
struct WTCallback : public std::enable_shared_from_this<WTCallback>,
 ...
 public:
  ...
  static std::shared_ptr<WTCallback> create(evpp::EventLoop* base) {
 ...
 protected:
  evpp::EventLoop* base_;
  std::shared_ptr<Promise<Unit>> promise_;

  explicit WTCallback(evpp::EventLoop* base)
      : base_(base) {
...
ThreadWheelTimekeeper::ThreadWheelTimekeeper()
    : eventBase_(),
      wheelTimer_(
          HHWheelTimer::newTimer(&eventBase_, std::chrono::milliseconds(1))) ,thread_([this]{eventBase_.Run();}){
...
Future<Unit> ThreadWheelTimekeeper::after(Duration dur) {
  ...
  // fine here.
  //
  eventBase_.RunInLoop([this, cob, dur]{
        wheelTimer_->scheduleTimeout(cob.get(), dur);
      });
  /*if (!eventBase_.RunInLoop([this, cob, dur]{
        wheelTimer_->scheduleTimeout(cob.get(), dur);
      })) {
...
Timekeeper* getTimekeeperSingleton() {
	//static Timekeeper*  instance = std::make_shared<ThreadWheelTimekeeper>();
	static Timekeeper*  instance = new ThreadWheelTimekeeper();
	/*static std::mutex mtx;
	if (instance == nullptr) {
		std::lock_guard<std::mutex> g(mtx);
		if (instance == nullptr) {
			instance = new ThreadWheelTimekeeper();
		}
	}*/
  return instance;
}
