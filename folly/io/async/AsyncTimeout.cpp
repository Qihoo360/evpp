...
/*AsyncTimeout::AsyncTimeout(evpp::EventLoop* eventBase) {
  assert(eventBase);
...
/*AsyncTimeout::AsyncTimeout(evpp::EventLoop* eventBase, InternalEnum internal) {

  folly_event_set(
      &event_, -1, EV_TIMEOUT, &AsyncTimeout::libeventCallback, this);
  //event_.ev_base = nullptr;
  event_base_set(eventBase->event_base(), &event_);
  //timeoutManager_->attachTimeoutManager(this, internal);
}*/
