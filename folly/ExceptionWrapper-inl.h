...
template <class Ex>
inline void exception_wrapper::InPlace<Ex>::throw_(
    exception_wrapper const* that) {
...
inline void exception_wrapper::SharedPtr::throw_(
    exception_wrapper const* that) {
...
inline std::string exception_wrapper::what() const {
...
inline std::string exception_wrapper::class_name() const {
    auto& ti = type();
    return ti == none()
           ? ""
           : ti == unknown() ? "<unknown exception>" : ti.name();
}
...
inline void exception_wrapper::throw_exception() const {
    vptr_->throw_(this);
    onNoExceptionError();
}
...
struct exception_wrapper::HandleReduce {
    ...
    std::function<void()> operator()(ThrowFn&& th, CatchFn& ca) const  {
...
struct exception_wrapper::HandleStdExceptReduce {
    ...
    template <
        class ThrowFn,
        class CatchFn, class Arg,
        FOLLY_REQUIRES(!IsCatchAll<CatchFn>::value) >
    auto operator()(ThrowFn&& th, CatchFn& ca) const ->std::function < StdEx* (Arg&&) > {
        using Ex = _t<ExceptionTypeOf<CatchFn, IsConst>>;
        return [ th = std::forward<ThrowFn>(th), &ca ](Arg && continuation)
    -> StdEx* {
    ...
    template <
        ...
        FOLLY_REQUIRES(IsCatchAll<CatchFn>::value) >
    auto operator()(ThrowFn&& th, CatchFn& ca) const -> std::function < StdEx* (Arg&&) > {
    return [ th = std::forward<ThrowFn>(th), &ca ](Arg&&) -> StdEx* {
...
template <class This, class Arg, class... CatchFns>
inline void exception_wrapper::handle_(
...
struct catch_fn {
    Fn fn_;
    auto operator()(Ex& ex) ->decltype(fn_(ex)) {
        return fn_(ex);
    }
};
