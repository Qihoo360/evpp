...
static void OnTestPrefixDone(const std::string& prefix_key, const PrefixGetResult& res) {
...
// TEST_UNIT(testMemcacheClient) {
   ...
   for (size_t i = 1; i < 100/*MAX_KEY*/; ++i) {
       std::stringstream ss;
       ss << "test" << i;
       usleep(1000);
       //mcp.PrefixGet(g_loop, ss.str(), &OnTestPrefixDone);
      mcp.Get(g_loop, ss.str(), &OnTestGetDone);
   }

  std::vector<std::string> mget_keys;
  for(size_t i = 0; i < 10/*MAX_KEY*/; ++i) {
      std::stringstream ss;
      ss << "test" << i;
